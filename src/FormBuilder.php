...
class FormBuilder
{
    ...
    /**
     * Create a select box field with predefined placeholder when value is not present.
     *
     * @param string $name
     ...
     * @return \Illuminate\Support\HtmlString
     */
    public function selectWithPlaceholder($name, $list = [], $selected = null, $options = [])
    {
        // When building a select box the "value" attribute is really the selected one
        // so we will use that when checking the model or session for a value which
        // should provide a convenient method of re-populating the forms on post.
        ...
        // them until we have an array of HTML declarations. Then we will join them
        // all together into one single HTML element that can be put on the form.
        $html = ['<option disabled' . (is_null($selected)? ' selected': '') . '>Select one</option>'];
